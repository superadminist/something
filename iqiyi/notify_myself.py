...
def wecom_video(title: str, content: str) -> None:
    """
    通过 企业微信 APP 推送消息。
    """
    if not push_config.get("QYWX_VIDEO"):
        print("QYWX_VIDEO 未设置!!\n取消推送")
        return
    QYWX_VIDEO_AY = re.split(",", push_config.get("QYWX_VIDEO"))
    if 4 < len(QYWX_VIDEO_AY) > 5:
        print("QYWX_VIDEO 设置错误!!\n取消推送")
        return
    print("企业微信 APP 服务启动")

    corpid = QYWX_VIDEO_AY[0]
    corpsecret = QYWX_VIDEO_AY[1]
    touser = QYWX_VIDEO_AY[2]
    agentid = QYWX_VIDEO_AY[3]
    try:
        media_id = QYWX_VIDEO_AY[4]
    except IndexError:
        media_id = ""
    wx = WeCom(corpid, corpsecret, agentid)
    # 如果没有配置 media_id 默认就以 text 方式发送
...
if push_config.get("QYWX_VIDEO"):
    notify_function.append(wecom_video)
