...
class IQIYI():
    ...
    def user_information(p00001):
        ...
        if res["code"] == "A00000":
            try:
                ...
                # 预计还需几天升级
                if today_growth_value > 0:
                    estimated_days = distance // today_growth_value  # 使用整除避免小数
                else:
                    estimated_days = "无法估算"  # 如果 today_growth_value 为 0，则无法估算
                msg = [
                    ...
                    {"name": "预计升级还需", "value": estimated_days if isinstance(
                        estimated_days, str) else f"{estimated_days} 天"}
    ...
    def main(self):
        ...
        for _ in range(3):
            ...
            task_msg = self.get_task_rewards(
                p00001=p00001, task_list=task_list)
...
if __name__ == "__main__":
    ...
    with open(
        ...
        iqiyi_data = datas.get("IQIYI", [])
        for _check_item in iqiyi_data:
            result = IQIYI(check_item=_check_item).main()
            print(result)
    # _check_item = datas.get("IQIYI", [])[0]
    # print(datas.get("IQIYI", []))
